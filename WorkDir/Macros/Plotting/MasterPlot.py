...
#
    ...
    #signalFiles.append('/user/hteagle/AnalysisDirectory/Rel21/Base.21.2.31/run/submitdir_ttbarCheck/data-output/mc16_13TeV.root')
...
#cutstouse = "(ETMiss>=100)*(nLeptons==1)*(pTl1>27)*(nBJets)*(pTb1>40)*(pTb2>25)*(pTj1>50)*(pTj2>40)*(minDPhijMET_4>0.4)*(m_T>120)"#
...
#ShapePlots
    ...
    if options.doCutflow:
        ...
        ##untrimmed
        signalFile = '/user/hteagle/AnalysisDirectory/Rel21/Base.21.2.31/run/submitdir_ttbarCheck/data-output/mc16_13TeV.root'
...
#Significances
    if options.doSignif:
        ...
        preSelection_Wh_ML_1L= cleaningCuts+"(met>100)*(mt>50)*(nLep_signal==1)*(nBJet25_MV2c10==2)*(nJet25<4)*(lep1Pt>27)"

        if label == "preSelection_Wh_ML_1L":
            cutstouse = preSelection_Wh_ML_1L
        ##ML selections
        elif label == 'XGB_0.999':
            cutstouse = "("+preSelection_Wh_ML_1L+"(xgboost_tune6_class2>0.999))"
        ...
        directory = "/user/hteagle/liverpool-ml/TMVATuples/reco_train/"
        ...
        Significance.Significance(cutstouse,luminosity_sim ,signalFiles, bkgFile, outputdir, label)
