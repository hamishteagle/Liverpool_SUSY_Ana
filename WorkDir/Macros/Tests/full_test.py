...
class Livana(unittest.TestCase):
    ...
    def write_cutflow(self, branch="CollectionTree_PFlow_"):
        ...
        flow_dict[branch]=collections.OrderedDict()
        ...
        for var, val in cuts_dict.items():
            ...
            flow_dict[branch][var] = self.t.GetEntries(str(cutnow)),weighted_hist.Integral()
            del weighted_hist
            if (flow_dict[branch][var][0]==0):
                raise IOError('cut of "{0:s} {1:s}" is too tight (0 events) change this cut'.format(var, val))
            else:
                print(self.blue(" {0:<20} | {1:^60}  | {2:^10}  | {3:>15}  ".format(str(branch), "cut: "+str(var)+" at: "+str(cuts_dict[var])," raw: "+str(flow_dict[branch][var][0])," weighted: "+str(flow_dict[branch][var][1]))))
    ...
    def get_runfile(self, branch_number=0):
        ...
        self.branch_list =[key.GetName() for key in self.f.GetListOfKeys() if isinstance(key.ReadObj(),ROOT.TTree)]
    ...
    def test_weights(self):
        ...
        for weight in weights_dict:
            ...
            if not os.path.exists(os.path.join(Livana.__location__,"WeightPlots",self.branch_list[i])):
                os.mkdir(os.path.join(Livana.__location__,"WeightPlots",self.branch_list[i]))
            saveString =  os.path.join(Livana.__location__,"WeightPlots",self.branch_list[i],weight+'.pdf')
            ...
            del h_current
            del c
            self.write_weights(branch = self.branch_list[i])
        if not Livana.args["force"]:
