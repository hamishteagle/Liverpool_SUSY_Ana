...
int main(int argc, char *argv[])
{
  ...
  if (debug)
    info_message("Running with message level DEBUG");
  if (doCombiLeptons)
  ...
  if (!RunningLocally)
    info_message("--- Going to submit to grid ---");
  if (NoEvents != -1)
    info_message("Events: " + std::to_string(NoEvents));
  ...
  if (RunningLocally)
  {
    ...
    if (NoEvents != -1)
      job.options()->setDouble(EL::Job::optMaxEvents, NoEvents);
