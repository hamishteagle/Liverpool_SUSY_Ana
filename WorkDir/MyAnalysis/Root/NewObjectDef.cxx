...
NewObjectDef::NewObjectDef(asg::SgTEvent *event, ST::SUSYObjDef_xAOD *SUSYTool, xAOD::TStore *store, double mcChannel, double EventNumber, double mcWgt, double m_lumiScaled, std::string syst, bool doTruthJets, int m_doCombiLeptons)
{
  ...
  if (doTruthJets)
  {
    goodTruthJets = std::make_unique<xAOD::JetContainer>(SG::VIEW_ELEMENTS);
    this->GetTruthJets();
    eventStore->record(goodTruthJets.release(), "goodTruthJets");
  }
}
...
void NewObjectDef::GetBaselineObjects()
{
  ...
  for (const auto &ph_itr : *photons)
  {
    if (ph_itr->auxdata<char>("baseline"))
      preOR_baselinePhotons->push_back(ph_itr);
  }
  //Fill preOR jets
  for (const auto &jet_itr : *jets)
  {
    if (jet_itr->auxdata<char>("baseline") && jet_itr->pt() * 0.001 > 30)
      preOR_baselineJets->push_back(jet_itr);
  }
  //Fill preOR taus
  for (const auto &tau_itr : *taus)
  {
    if (tau_itr->auxdata<char>("baseline"))
      preOR_baselineTaus->push_back(tau_itr);
  }
  if (doCombiLeptons)
  {
    ...
    {
      if (mu_itr->auxdata<char>("baseline") && mu_itr->pt() * 0.001 > 6 && TMath::Abs(mu_itr->eta()) < 2.5)
        preOR_baselineMuons->push_back(mu_itr);
    }
    for (const auto &el_itr : *electrons)
    {
      if (el_itr->auxdata<char>("baseline") && el_itr->pt() * 0.001 > 7 && TMath::Abs(el_itr->eta()) < 2.47)
        preOR_baselineElectrons->push_back(el_itr);
    }
    for (const auto &el_itr : *electrons)
    {
      if (el_itr->auxdata<char>("baseline"))
        preOR_baselineElectrons_combi->push_back(el_itr);
    }
    //Fill preOR combi muons
    for (const auto &mu_itr : *muons)
    {
      if (mu_itr->auxdata<char>("baseline"))
        preOR_baselineMuons_combi->push_back(mu_itr);
  ...
  //Clean up the MET containers which we can't make unique
  delete met_nominal;
...
void NewObjectDef::GetObjects()
{
  ...
  for (const auto &mu_itr : *preOR_baselineMuons)
  {
    if (mu_itr->auxdata<char>("passOR"))
    {
      ...
      if (mu_itr->auxdata<char>("baseline") && mu_itr->auxdata<char>("cosmic"))
  ...
  for (const auto &tau_itr : *preOR_baselineTaus)
  {
    ...
    {
      if (tau_itr->auxdata<char>("baseline"))
        baselineTaus->push_back(tau_itr);
      if (tau_itr->auxdata<char>("signal"))
        goodTaus->push_back(tau_itr);
    }
  }
  ...
  for (const auto &jet_itr : *preOR_baselineJets)
  {
    ...
    {
      if (jet_itr->auxdata<char>("bad"))
        badJets->push_back(jet_itr);
      if (jet_itr->auxdata<char>("baseline"))
        baselineJets->push_back(jet_itr);
      if (jet_itr->auxdata<char>("signal"))
        goodJets->push_back(jet_itr);
      if (jet_itr->auxdata<char>("signal") && jet_itr->auxdata<char>("bjet") >= 3)
        BJets->push_back(jet_itr);
      else if (jet_itr->auxdata<char>("signal") && (jet_itr)->auxdata<char>("bjet") < 3)
