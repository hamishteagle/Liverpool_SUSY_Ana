...
CalculateVariables::CalculateVariables(NewObjectDef *objects, xAOD::TStore *evtStore, bool isTruth, bool doPhotons, bool isData, bool doTruthJets, std::string systematic)
{
  ...
  if (nJets > 0)
    all_HT = 0; //start this at zero properly if we are going to calculate it.
  for (int iJet = 0; iJet < nJets; iJet++)
  {
  ...
  if (nJets >= 1)
  {
    ...
    if (nJets >= 2)
    {
      ...
      if (nJets >= 3)
      {
        ...
        if (nJets >= 5)
          pTj5 = (*(goodJet_cont))[4]->pt() * 0.001;
        if (nJets >= 6)
          pTj6 = (*(goodJet_cont))[5]->pt() * 0.001;
        if (nJets >= 7)
          pTj7 = (*(goodJet_cont))[6]->pt() * 0.001;
        if (nJets >= 8)
          pTj8 = (*(goodJet_cont))[7]->pt() * 0.001;
      }
    }
  }
  ...
  if (nbJets >= 1)
  {
    ...
    if (!isData)
    {
      (*(bJet_cont))[0]->getAttribute("ConeTruthLabelID", flavour);
    }
    truthFlavb1 = flavour;
    //
  }
  if (nbJets >= 2)
  {
    ...
    if (!isData)
    {
      (*(bJet_cont))[1]->getAttribute("ConeTruthLabelID", flavour);
    }
    truthFlavb2 = flavour;
    //
  }
    ...
    if (((*(goodJet_cont))[iJet]->auxdata<char>("bjet") != true) && fabs((*(goodJet_cont))[iJet]->eta()) < 2.5)
    {
      ...
      if (ntrk.size() > 0)
      {
        if (ntrk[0] <= 4)
        {
          ...
          if (fabs(dphi) < (TMath::Pi() / 5.0))
          {
            Stop0L_tauVeto = false;
          }
        }
      }
    }
  ...
  for (int i = 0; i < (bJet_cont->size()); ++i)
  {
    for (int j = 0; j < bJet_cont->size(); ++j)
    {
      ...
      //minimisation with exclusion
      {
        ...
        if (nbJets == 3)
        {
          std::cout << "something weird here" << std::endl;
        }
      }
...
void CalculateVariables::CalculateTwoLepVariables(NewObjectDef *objects, TLorentzVector b1v, TLorentzVector b2v)
{
  ...
  //lepton pTs
  if (nElectron > 0)
    pTel1 = (*goodElectron_cont)[0]->pt() * 0.001;
  if (nElectron > 1)
    pTel2 = (*goodElectron_cont)[1]->pt() * 0.001;
  if (nMuon > 0)
    pTmu1 = (*goodMuon_cont)[0]->pt() * 0.001;
  if (nMuon > 1)
    pTmu2 = (*goodMuon_cont)[1]->pt() * 0.001;
  ...
  if (pTl1 < pTl2)
  {
    std::cout << "The lepton pT stuff is screwed" << std::endl;
  }
