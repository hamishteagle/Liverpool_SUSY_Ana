...
EL::StatusCode MyxAODAnalysis ::initialize()
{
  ...
  if (isMC16d)
  {
    lumicalcFiles.push_back(PathResolverFindCalibFile("GoodRunsLists/data17_13TeV/20180619/physics_25ns_Triggerno17e33prim.lumicalc.OflLumi-13TeV-010.root"));
  }
  if (isMC16e)
  {
  ...
  for (const auto &output_tree_string : output_trees)
  {
    for (const auto &sysInfo : systInfoList)
    {
      ...
      if (isMC)
      {
        ...
        {
          if (counter == 1)
            Error("execute()", "PMGCrossSectionTool exception caught");
        }
      ...
      auto m_varCalc = std::make_unique<CalculateVariables>(objs.get(), store, isTruth, doPhotons, isData, doTruthJets, syst.name());
